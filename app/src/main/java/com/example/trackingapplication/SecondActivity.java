...
public class SecondActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        slideMenu.add(new ItemSlideMenu(R.drawable.ic_exit, "Log Out"));
        ...
        listViewSliding.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (position == 1 || position == 2) {
                    if (position == 2)
                        FirstActivity.sharedPreferences.edit().putBoolean("close", true).apply();
    ...
    class AsyncT extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... voids) {
            try {
                ...
                if (FirstActivity.sharedPreferences.getBoolean("close", false))
                    url = new URL("http://Api-env.pjxxtmeicp.us-east-2.elasticbeanstalk.com/api/track/close");
                else
                    url = new URL("http://Api-env.pjxxtmeicp.us-east-2.elasticbeanstalk.com/api/track/cleared");
                ...
                try (BufferedReader read = new BufferedReader(
                    ...
                    if (FirstActivity.sharedPreferences.getBoolean("close", false))
                        System.out.println("Close Response: " + jsonResponse.toString());
                    else
                    ...
                    if (FirstActivity.sharedPreferences.getBoolean("close", false)) {
                        if (numberResponse.contains("\"Code\":1")) {
                            responseCode = 1;
                            FirstActivity.sharedPreferences.edit().putBoolean("isFirstRun", true).apply();
                        } else {
                            responseCode = 0;
                        }
                    } else {
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            if (FirstActivity.sharedPreferences.getBoolean("close", false)) {
                if (responseCode == 1) {
                    ...
                    FirstActivity.sharedPreferences.edit().putBoolean("close", false).apply();
                    FirstActivity.sharedPreferences.edit().putBoolean("isFirstRun", true).apply();
                    FirstActivity.sharedPreferences.edit().putBoolean("isFirstLoc", true).apply();;
                    FirstActivity.sharedPreferences =getSharedPreferences("loginPrefs",android.content.Context.MODE_PRIVATE);
                    FirstActivity.sharedPreferences.edit().clear().apply();
